...
- **화면 캡쳐** — 현재 프레임을 원본 해상도 PNG로 바로 저장 (번호판 신고용)
- **프레임 단위 이동** — `←` / `→`로 정확히 한 프레임씩, `Shift`와 함께 누르면 1초씩
  (파일마다 `requestVideoFrameCallback`으로 실제 fps를 측정해 24/29.97/30/60fps를 그대로 따라갑니다)
